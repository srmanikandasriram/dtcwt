...
def _as_row_tensor(h):
    """Convert input to a row tensor of shape [1, N]."""
...
def _as_col_tensor(h):
    """Convert input to a column tensor of shape [N, 1]."""
    if isinstance(h, torch.Tensor):
        h = h.reshape(-1, 1)
    else:
...
def _conv_2d(X, h, stride=(1, 1)):
    """
    Perform 2d convolution in PyTorch.

    X will be manipulated to be of shape [batch, ch, height, width],
    and h to be of shape [num, ch, height, width]. This function does the
    ...
    X_in = X.unsqueeze(1)  # [batch, 1, h, w]
    h_in = h.unsqueeze(0).unsqueeze(0)  # [1, 1, h_h, h_w]

    # PyTorch conv2d does correlation by default, need to flip filter for convolution
    h_in = torch.flip(h_in, dims=[2, 3])

    # Perform convolution
    Y = F.conv2d(X_in, h_in, stride=stride, padding=0)

    # Remove the channel dimension, returning a result of shape [batch, height, width]
    Y = Y.squeeze(1)
...
def colfilter(X, h, align=False):
    ...
    if m % 2 == 0 and align:
        X = _torch_pad(X, [[0, 0], [m2 - 1, m2], [0, 0]], mode='reflect')
    else:
        X = _torch_pad(X, [[0, 0], [m2, m2], [0, 0]], mode='reflect')

    Y = _conv_2d(X, h_t, stride=(1, 1))
...
def rowfilter(X, h, align=False):
    ...
    # Symmetrically extend with repeat of end samples.
    # Pad only the third dimension of the tensor X (the rows)
    if m % 2 == 0 and align:
        X = _torch_pad(X, [[0, 0], [0, 0], [m2 - 1, m2]], mode='reflect')
    else:
        X = _torch_pad(X, [[0, 0], [0, 0], [m2, m2]], mode='reflect')
...
def rowdfilt(X, ha, hb, no_decimate=False):
    ...
    c2 = c // 2
    if c % 4 != 0:
        raise ValueError('No. of columns in X must be a multiple of 4\n' +
                         'X was {}'.format(X.shape))
    ...
    # Symmetrically extend with repeat of end samples.
    # Pad only the third dimension of the tensor X (the rows).
    X = _torch_pad(X, [[0, 0], [0, 0], [m, m]], mode='reflect')

    # Take the odd and even columns of X
    X_odd = X[:, :, 2:c + 2 * m - 2:2]
    X_even = X[:, :, 3:c + 2 * m - 2:2]

    a_cols = _conv_2d(X_odd, ha_t, stride=(1, 2))
    b_cols = _conv_2d(X_even, hb_t, stride=(1, 2))

    # Stack a_cols and b_cols along dimension 3
    if torch.sum(ha_t * hb_t) > 0:
        Y = torch.stack([a_cols, b_cols], dim=3)
    else:
        Y = torch.stack([b_cols, a_cols], dim=3)
...
def colifilt(X, ha, hb, no_decimate=False):
    ...
    Y = torch.stack([y1, y2, y3, y4], dim=2)
...
def rowifilt(X, ha, hb, no_decimate=False):
    """
    Filter the rows of image X using the two filters ha and hb = reverse(ha).

    This performs the inverse operation to rowdfilt.
    ...
    r, c = X.shape[1:]
    if c % 2 != 0:
        raise ValueError('No. of columns in X must be a multiple of 2.\n' +
                         'X was {}'.format(X.shape))
    ...
    # Reshape to interleave the columns
    Y = Y.reshape(-1, r, 2 * c)
