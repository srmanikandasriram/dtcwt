...
from dtcwt.torch.lowlevel import coldfilt, colfilter, colifilt

try:
    import torch
    torch_dtypes = frozenset(
...
class Transform1d(object):
    """
    An implementation of the 1D DT-CWT in PyTorch.
    ...
    .. note::
        ...
        :py:meth:`~dtcwt.torch.Transform1d.forward` or
        :py:meth:`~dtcwt.torch.Transform1d.forward_channels` methods with a numpy
        array, they load this array into a :py:class:`torch.Tensor` and create
        the graph. Subsequent calls to :py:attr:`dtcwt.torch.Pyramid.lowpass` or
        other attributes in the pyramid will convert the tensors back to numpy.
        If the above methods are called with a PyTorch tensor, these will be used
        directly. As such, to evaluate the results, you will need to look at the
        :py:attr:`dtcwt.torch.Pyramid.lowpass_op` attribute.

        The behaviour is similar for the
        :py:meth:`~dtcwt.torch.Transform1d.inverse` and
        :py:meth:`~dtcwt.torch.Transform1d.inverse_channels` methods, except these
        return an array, rather than a Pyramid style class. If a
        :py:class:`dtcwt.torch.Pyramid` was created by calling the forward methods
        with a numpy array, providing this pyramid to the inverse methods will
        return a numpy array. If however a :py:class:`dtcwt.torch.Pyramid` was
        created by calling the forward methods with a PyTorch tensor, the
        result from calling the inverse methods will also be a PyTorch tensor.
    """
    ...
    def forward(self, X, nlevels=3, include_scale=False):
        ...
        or a PyTorch tensor (designed usage). To transform batches of vectors,
        use the :py:meth:`forward_channels` method.

        :param X: 1D real array or 2D real array whose columns are to be
            transformed.
        :param nlevels: Number of levels of wavelet decomposition

        :returns: A :py:class:`dtcwt.torch.Pyramid` object representing the
            transform result.
        ...
        *biort* case, this should be (h0o, g0o, h1o, g1o). In the *qshift* case,
        ...
        elif dtype in torch_dtypes:
            if len(X.shape) == 1:
                X = X.unsqueeze(-1)
        else:
            raise ValueError('I cannot handle the variable you have ' +
                             'provided of type ' + str(X.dtype) + '. ' +
                             'Inputs should be a numpy or torch array')

        X_shape = tuple(X.shape)
        if len(X_shape) == 2:
            # Need to make it a batch for PyTorch
            X = X.unsqueeze(0)
        elif len(X_shape) >= 3:
            raise ValueError(
    ...
    def forward_channels(self, X, nlevels=3, include_scale=False):
        ...
        Can provide the forward transform with either an np array (naive usage),
        or a PyTorch tensor (designed usage).
    ...
    def inverse(self, pyramid, gain_mask=None):
        ...
        :returns: Reconstructed real array. Will be a torch.Tensor if the Pyramid
            was made with torch inputs, otherwise a numpy array.
        ...
        # Check if a numpy pyramid was provided
            ...
            Yh = tuple(
                torch.from_numpy(level).cfloat()
                for level in Yh)
        else:
            raise ValueError(
                'Unknown pyramid provided to inverse transform')

        # Need to make sure it has at least 3 dimensions for PyTorch
        Yl_shape = tuple(Yl.shape)
        if len(Yl_shape) == 2:
            Yl = Yl.unsqueeze(0)
            Yh = tuple(x.unsqueeze(0) for x in Yh)
        elif len(Yl_shape) >= 3:
            raise ValueError(
    ...
    def inverse_channels(self, pyramid, gain_mask=None):
        ...
        :py:meth:`~dtcwt.torch.Transform1d.forward_channels` method.
        ...
        :returns: Reconstructed array. Will be a torch.Tensor if the Pyramid was
            made with torch inputs, otherwise a numpy array.
        ...
        if isinstance(pyramid, Pyramid):
            ...
            if len(Yl_shape) != 3:
                raise ValueError(
                    'Incorrect input shape for the inverse_channels ' +
                    'method ' + str(Yl_shape) + '. For Inputs of 1 or 2 ' +
                    'dimensions, use the inverse method.')
            Yh = pyramid.highpasses_ops
            numpy = pyramid.numpy
    ...
    def _forward_ops(self, X, nlevels=3):
        """ Perform a *n*-level DTCWT-1D decompostion.
        ...
        if not isinstance(X, torch.Tensor):
            raise ValueError('X needs to be a torch tensor')

        original_size = list(X.shape[1:])
        ...
        if nlevels >= 1:
            ...
            Yh[0] = Hi[:, ::2, :].to(torch.complex64) + \
                1j * Hi[:, 1::2, :].to(torch.complex64)
            Yscale[0] = Lo
        ...
        for level in xrange(1, nlevels):
            ...
            if Lo.shape[1] % 4 != 0:
                Lo = torch.nn.functional.pad(Lo, (0, 0, 1, 1), mode='reflect')
            ...
            # Convert Hi to complex form by taking alternate rows
            Yh[level] = Hi[:, ::2, :].to(torch.complex64) + \
                1j * Hi[:, 1::2, :].to(torch.complex64)
            Yscale[level] = Lo
    ...
    def _inverse_ops(self, Yl, Yh, gain_mask=None):
        ...
        :returns: A torch.Tensor holding the output
        ...
        while level >= 1:
            ...
            # If Lo is not the same length as the next Yh => it was extended.
            # Therefore we have to clip Lo so it is the same height as the next Yh.
            Lo_shape = Lo.shape
            next_shape = Yh[level - 1].shape
            ...
            if (np.any(np.asanyarray(Lo_shape[1:]) !=
                       np.asanyarray([next_shape[1] * 2, next_shape[2]]))):
...
def c2q1d(x):
    ...
    """
    # Input has shape [batch, r, c]
    r, c = x.shape[1:3]
    x1 = torch.real(x)
    x2 = torch.imag(x)
    # Stack 2 inputs of shape [batch, r, c] to [batch, r, 2, c]
    y = torch.stack([x1, x2], dim=2)
